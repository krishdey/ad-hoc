 ...
 * It is also configurable with a start and time as well as a specification of
 * the region server implementation if different from the local cluster.
 */
...
public class CopyTable extends Configured implements Tool {
	...
	String nodeOrNodeLink = null;
	boolean allCells = false;
	static boolean shuffle = false;

	boolean bulkload = false;
	boolean renamerow = false;
	...
	public Job createSubmittableJob(String[] args) throws IOException {
		...
		if (bulkload) {
			...
			// We need to split the inputs by destination tables so that output of Map can
			// be bulk-loaded.
			...
			// Added newhash flag for renaming the rowkey
		} else if (renamerow) {

			if (nodeOrNodeLink.equals("node")) {
				TableMapReduceUtil.initTableMapperJob(tableName, scan, RowKeyRenameImporter.NodeKeyRenameImport.class,
						null, null, job);
			} else {
				TableMapReduceUtil.initTableMapperJob(tableName, scan, RowKeyRenameImporter.NodeLinkRenameImport.class,
						null, null, job);
			}
	...
	private static void printUsage(final String errorMsg) {
		...
		System.err.println("Usage: CopyTable [general options] [--starttime=X] [--endtime=Y] "
				+ "[--new.name=NEW] [--peer.adr=ADR] [--renamerow=node or nodelink] <tablename>");
		System.err.println();
	...
	private boolean doCommandLine(final String[] args) {
		...
		try {
			for (int i = 0; i < args.length; i++) {
				...
				final String renameRow = "--renamerow=";
				if (cmd.startsWith(renameRow)) {
					nodeOrNodeLink = cmd.substring(renameRow.length());
					renamerow = true;
					continue;
				}
	...
	public int run(String[] args) throws Exception {
		...
		if (bulkload) {
			...
			if (code == 0) {
				// bulkloadDir is deleted only LoadIncrementalHFiles was successful so that one
				// can rerun
		...
		if (renamerow) {
			if (!(nodeOrNodeLink.equals("node") || nodeOrNodeLink.equals("nodelink"))) {
				printUsage("Invalid table name: can only be node or nodelink");
				code = 1;
			}
		}
		return code;
